...
export function YearViewWeb({
  ...
  onAdd?: () => void;
  onOpenEdit?: (event: SharedEvent) => void;
}) {
  ...
  const openHighlight = (item: LocalHighlight) => {
    const sourceEvent = events.find((event) => event.id === item.id);
    if (sourceEvent && onOpenEdit) {
      onOpenEdit(sourceEvent);
      return;
    }
  ...
  return (
    <div className="px-5 pt-5 pb-32 lg:px-4 lg:pt-7 xl:px-2">
      <div style={{ fontSize: 16, fontWeight: 700, color: accent, lineHeight: 1.1 }}>{year}</div>
      <div
        className="mt-1"
        style={{
          fontSize: 40,
      ...
      <div style={{ fontSize: 17, fontWeight: 600, letterSpacing: "-0.2px" }} className="text-[var(--text-secondary)] mt-3">
        119일 / 365일 · 33%
      </div>
      <div className="h-1.5 rounded-full mt-4 overflow-hidden" style={{ background: "var(--bg-tertiary)" }}>
        <div className="h-full rounded-full" style={{ width: "33%", background: accent }} />
      </div>

      <div className={`mt-7 ${isDesktop ? "space-y-4" : "space-y-3"}`}>
